...
import Cookies, { set } from 'js-cookie'  //可以操作前端cookie的对象
...
function Lockscreen(props) {
    ...
    let userid;
         ...
         useEffect(() => {
            ...
            (async() => {
                // 读取cookie中的userid
                  userid = await Cookies.get('username');
                 Cookies.set('secret',props.location.query);
             // 如果没有，自动重定向到登录页面
          if (!userid) {   
...
// 改变音乐播放的暂停/开始按键
   ...
   //
    useEffect(()=>{
       window.document.getElementsByClassName('rnd')[0].style.zIndex=-1;
    //    console.log(document.getElementsByClassName('rnd')[0].children[1]);
    //    let targetwindow=document.getElementsByClassName('rnd')[0].children[1];
    //    console.log(targetwindow.contentWindow);
    //    console.log(targetwindow.contentWindow.document.body);
    //    targetwindow.contentWindow.postMessage({cookie:userid},'http://localhost:3001/');

    },[])

    const seek= async(e)=>{
        if(e.target.className === 'willchange'){
         switch(e.target.name){
           case 'open':
                     window.document.getElementsByClassName('rnd')[0].style.zIndex=1;
                     e.target.name='willchange3-colse-narrow';
                     return false;
            case 'willchange3-colse-narrow':
                     return false;
            case 'close':
                     window.document.getElementsByClassName('rnd')[0].style.zIndex=1;
                     window.document.getElementsByClassName('iframe01')[0].src='http://localhost:3001/';
                     e.target.name='willchange3-colse-narrow';
                     return false;
            case 'narrow':
                     window.document.getElementsByClassName('rnd')[0].style.zIndex=1;
    ...
    let [state1,setState1]=new useState({x:100,y:100,width:940,height:500});
      ...
      return (
        ...
        <div
            ...
            style={{ position:'absolute',backgroundImage: "url(" + require("../../assets/images/wallpaper-day.jpg").default + ")",  }}
        ...
        >
            ...
            <div className='bottom' >
                <ul>
                    ...
                    <li className='ml-end3' >
                        <img src={safari} style={{cursor:'pointer'}} name='open' onClick={seek} alt="搜索"  className='willchange'/>
                    </li>
                    <li className='ml-end'>
